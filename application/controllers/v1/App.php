...
class App extends CI_Controller { 
	...
	public function register() {
			...
			if(!empty($first_name) && !empty($last_name) && !empty($email) && !empty($phone) && !empty($password)) {
		 	$users = $this->home_db->getRecords('users',['emailid'=>$email],'*');
		 	...
		 	if(count($users) >0) {
		 		...
		 		$insert = $this->home_db->insertRecord('users',$insertDB);
		 		if($insert) {
		 			$userdata = ['user_id'=>$insert,'name'=>$first_name,'email'=>$email,'phone'=>$phone];
		 			$result = array('status'=>'success','register_data'=>$userdata);
		 		}
		 	}
		}
...
public function reviews() {
	...
	public function myOrders() {
		...
		public function profileDetails() {
			...
			$user_id = trim(preg_replace('!\s+!', '',$data['user_id']));
			if(!empty($user_id)) {
			$list = $this->home_db->getRecords('users',['id'=>$user_id],'id,name,sname,phone,emailid,address1,address2,country,town,state,town_name,state_name,zip');
			//echo $this->db->last_query();exit;
			if(count($list)) {
				$userdata = ['user_id'=>$list[0]->id,'first_name'=>$list[0]->name,'last_name'=>$list[0]->sname,'phone'=>$list[0]->phone,'email'=>$list[0]->emailid,'billing_address_1'=>$list[0]->address1,'billing_address_2'=>$list[0]->address2,'country_id'=>$list[0]->country,'town_id'=>$list[0]->town,'state_id'=>$list[0]->state,'town_name'=>$list[0]->town_name,'state_name'=>$list[0]->state_name,'pincode'=>$list[0]->zip];
		...
		public function updateProfile() {
			...
			if(!empty($user_id) && !empty($first_name) && !empty($last_name) && !empty($phone) && !empty($billing_address_1) && !empty($billing_address_2) && !empty($state) && !empty($pincode) && !empty($city) && !empty($state_id) && !empty($city_id)) {
				$update = $this->home_db->updateRecord('users',['name'=>$first_name,'sname'=>$last_name,'phone'=>$phone,'address1'=>$billing_address_1,'address2'=>$billing_address_2,'town'=>$city_id,'state'=>$state_id,'town_name'=>$city,'state_name'=>$state,'zip'=>$pincode,'modified_date'=>date('Y-m-d H:i:s')],'id',$user_id);
				if($update) {
		...
		public function confirmOrders() {
			...
			if(!empty($user_id) && !empty($state_id) && !empty($city_id) && !empty($bzip) && !empty($baddress1) && !empty($baddress2)  && !empty($sphone) && !empty($bphone) && !empty($payment_type) && !empty($szip)  &&  !empty($total_amt) && !empty($saddress1) && !empty($saddress2) && !empty($sname) && !empty($bemail) && !empty($bname) && is_array($product)) {
				...
				if(count($user_exists)) {
					...
					$oid = $this->home_db->insertRecord('orders',$orders);
					if($oid) {                    
                	...
                	$obs = $this->home_db->insertRecord('orders_bill_ship',$db);
                	if(is_array($product)) {
                		foreach ($product as $products) {
                			...
                			$op['status'] = 0;
                			$opdd = $this->home_db->insertRecord('orders_products',$op);
                		}
                	}
     ...
     //            		$dbp = array(
					...
					$paid_amount['easebuzz_merchantkey'] = MERCHANT_KEY;
					$paid_amount['easebuzz_saltkey'] = SALT;
					$paid_amount['currency_type'] = 'INR';
					$paid_amount['easebuzz_callback'] = base_url().'v1/easebuzzResponse';

					$txn = md5(rand(1234,9999));
					$amt = sprintf("%.2f",$total_amt);
					$mkey = "B86M0DKPUZ";
					$skey = "GABG4LRJG5";
					$sequence = hash('sha512',"$mkey|$txn|$amt|test|$bname|$bemail|||||||||||$skey");
			//echo "$mkey|$txn|$amt|test|$bname|$bemail|||||||||||$skey";exit;
					$ch = curl_init();
					curl_setopt($ch, CURLOPT_URL, 'https://pay.easebuzz.in/payment/initiateLink');
					curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
					curl_setopt($ch, CURLOPT_POST, 1);
					curl_setopt($ch, CURLOPT_POSTFIELDS, "key=$mkey&txnid=$txn&amount=$amt&productinfo=test&firstname=$bname&phone=$bphone&email=$bemail&surl=https://sriramamritatarangini.com/shop/v1/app/success&furl=https://sriramamritatarangini.com/shop/v1/app/failure&hash=$sequence");
					$headers = array();
					...
					if($decode['status'] ==1) {
						$paid_amount['data'] = $decode['data'];
					}
					else if($decode['status'] ==0) {
						$paid_amount['data']= "";
						$paid_amount['errorMessage']= $decode['data'];
					}
		...
		public function easebuzzResponse() {
			...
			$payment_id = trim(preg_replace('!\s+!', '',$razorresponse['payment_id']));
			$status = trim(preg_replace('!\s+!', '',$razorresponse['status']));
			$hash = trim(preg_replace('!\s+!', '',$razorresponse['hash']));
			$txn_id = trim(preg_replace('!\s+!', '',$razorresponse['txn_id']));
			$oid = trim(preg_replace('!\s+!', '',$razorresponse['oid']));
			if(!empty($payment_id) && !empty($status) && !empty($hash) && !empty($oid)) {
				if($status == "success") {
					  $dbp = array(
					'status' => 1,
					'pstatus'=>"success",
				...
				$this->home_db->updateRecord('orders',$dbp, 'orderid',$oid);
		...
		public function addWishlist() {
			...
			if(!empty($user_id) && !empty($pid) && !empty($cpage_url)) {
				$dbs=array(
	    				'cpage_url'=>$cpage_url,
	    				'pid'=>$pid,
	    				'user_id'=>$user_id,
		...
		public function categoryListView() {
			$categorywise = $this->home_db->getRecords('category',array('status'=>0),'id ,name,page_url as cpage_url','id asc');
			...
			foreach ($categorywise as $cat) {
				$id = $cat->id;
				 	...
				 	$subnew = [];
				 	 ...
				 	 $cat->productList = $newproducts;
			}
			echo json_encode(['status'=>'success','sliders'=>$banner,'category'=>$categorywise,'special_offers'=>$special_offers]);
		}
